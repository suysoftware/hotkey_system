#include "include/hotkey_system/hotkey_system_plugin.h"
...
#define HOTKEY_SYSTEM_PLUGIN(obj)                                     \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), hotkey_system_plugin_get_type(), \
                              HotkeySystemPlugin))
...
HotkeySystemPlugin* plugin_instance;
std::map<std::string, std::string> hotkey_id_map;

struct _HotkeySystemPlugin {
...
GtkWindow* get_window(HotkeySystemPlugin* self) {
...
G_DEFINE_TYPE(HotkeySystemPlugin, hotkey_system_plugin, g_object_get_type())
...
static FlMethodResponse* hkm_register(HotkeySystemPlugin* self,
                                      FlValue* args) {
...
static FlMethodResponse* hkm_unregister(HotkeySystemPlugin* self,
                                        FlValue* args) {
  const char* identifier =
      fl_value_get_string(fl_value_lookup_string(args, "identifier"));
...
static FlMethodResponse* hkm_unregister_all(HotkeySystemPlugin* self,
                                            FlValue* args) {
...
static void hotkey_system_plugin_handle_method_call(
    HotkeySystemPlugin* self,
...
static void hotkey_system_plugin_dispose(GObject* object) {
  G_OBJECT_CLASS(hotkey_system_plugin_parent_class)->dispose(object);
}

static void hotkey_system_plugin_class_init(HotkeySystemPluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = hotkey_system_plugin_dispose;
}

static void hotkey_system_plugin_init(HotkeySystemPlugin* self) {}

static void method_call_cb(FlMethodChannel* channel,
                           FlMethodCall* method_call,
                           gpointer user_data) {
  HotkeySystemPlugin* plugin = HOTKEY_SYSTEM_PLUGIN(user_data);
  hotkey_system_plugin_handle_method_call(plugin, method_call);
}

void hotkey_system_plugin_register_with_registrar(
    FlPluginRegistrar* registrar) {
  HotkeySystemPlugin* plugin = HOTKEY_SYSTEM_PLUGIN(
      g_object_new(hotkey_system_plugin_get_type(), nullptr));
  ...
  plugin->channel =
      fl_method_channel_new(fl_plugin_registrar_get_messenger(registrar),
                            "hotkey_system", FL_METHOD_CODEC(codec));
